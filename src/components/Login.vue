  ...
  <div class="auth-page" ref="pageRef">
    ...
    >
      ...
      <form @submit.prevent="submit" novalidate>
        ...
        <!-- email -->
        <div class="field" :class="{ 'field-error': errors.email, 'field-ok': !errors.email && form.email }">
          <label>{{ $t('Email') }}</label>
          ...
          <Transition name="shake-fade">
            <span v-if="errors.email" class="err-msg">⚠️ {{ errors.email }}</span>
          </Transition>
        </div>

        <!-- password -->
        <div class="field" :class="{ 'field-error': errors.password, 'field-ok': !errors.password && form.password }">
          ...
          <Transition name="shake-fade">
            <span v-if="errors.password" class="err-msg">⚠️ {{ errors.password }}</span>
          </Transition>
        </div>
        ...
        <button
          type="submit"
          class="btn-submit"
          :disabled="auth.loading || isLoading"
        >
      ...
      <router-link to="/" class="back-home">← {{ $t('Back to Home') || 'Back to Home' }}</router-link>
    </div>
...
.auth-card {
  ...
  /* entrance */
  opacity: 0;
  transform: perspective(900px) translateY(60px) rotateX(20deg) scale(.93);
}
.auth-card.in {
  opacity: 1;
  transform: perspective(900px) translateY(0) rotateX(0) scale(1);
  transition:
    opacity .8s cubic-bezier(.34,1.2,.64,1),
...
.field:nth-child(3) { animation-delay: .45s; }
...
.forgot-link:hover { color: #5dcaa5; }
...
.spinner-ring div:nth-child(2) { animation-delay: -0.30s; border-top-color: rgba(20,184,166,.5); }
.spinner-ring div:nth-child(3) { animation-delay: -0.15s; border-top-color: rgba(20,184,166,.25); }
