...
const submit = async () => {
  ...
  if (updated) {
    ...
    saveError.value = paymentStore.error || '❌ Failed to update payment.'
  }
}

/* ════════════════════════════════════════
   3D CANVAS — same engine as all pages
════════════════════════════════════════ */
...
<template>
  <div class="auth-page" ref="pageRef">
    ...
    <div class="auth-card" ref="cardRef" @mousemove="onMouseMove" @mouseleave="onMouseLeave">
      ...
      <!-- heading -->
      <h2 class="card-title">💰 {{ t('editPayment') }}</h2>
      <p class="card-sub">{{ t('editPaymentSubtitle') }}</p>

      <!-- alerts -->
      <Transition name="shake-fade">
        <div v-if="saveSuccess" class="success-alert">✅ Payment updated! Redirecting...</div>
      </Transition>
      <Transition name="shake-fade">
      ...
      <Transition name="shake-fade">
        <div v-if="paymentStore.loading" class="info-alert">
          <span class="mini-spin"></span>
          <span>{{ t('updatingPayment') || 'Updating payment...' }}</span>
        </div>
      </Transition>

      <!-- loading state -->
      <div v-if="!paymentForm" class="loading-msg">
        <span class="mini-spin"></span>
      ...
      <form v-else @submit.prevent="submit" novalidate>
        ...
        <div class="field">
          <label>📅 {{ t('month') }}</label>
          <div class="input-wrap">
            <input v-model="paymentForm.month" type="text" :placeholder="t('month')" required />
          </div>
        </div>

        <!-- year -->
        <div class="field">
          <label>🗓️ {{ t('year') }}</label>
          <div class="input-wrap">
            <input v-model="paymentForm.year" type="number" :placeholder="t('year')" required />
          </div>
        </div>
        ...
        <div class="field">
          ...
          <div class="input-wrap">
            <input v-model="paymentForm.amount" type="number" :placeholder="t('amount')" required />
          </div>
        </div>
        ...
        <!-- submit -->
        <button type="submit" class="btn-submit" :disabled="paymentStore.loading">
          <span v-if="!paymentStore.loading">💾 {{ t('saveChanges') }}</span>
          <span v-else class="btn-spinner"></span>
        </button>
      </form>

      <router-link to="/landlord" class="back-home"
        >← {{ t('back') || 'Back to Dashboard' }}</router-link
      >
    </div>
  </div>
</template>
...
.btn-submit {
  ...
  transition:
    ...
    box-shadow 0.3s;
  margin-top: 4px;
  animation: fadeUp 0.6s ease both 0.56s;
}
.btn-submit::before {
  content: '';
  position: absolute;
