...
const { announcementForm } = storeToRefs(announcementStore)
const { loadAnnouncementForEdit, updateAnnouncement } = announcementStore
...
onMounted(async () => {
  await loadAnnouncementForEdit(announcementId)
  initCanvas()
  buildCubes()
})
...
const submit = async () => {
  ...
  saveError.value = ''
  const updated = await updateAnnouncement(announcementId, announcementForm.value)
  if (updated) {
    saveSuccess.value = true
    setTimeout(() => router.push('/landlord'), 1200)
  } else {
    saveError.value = announcementStore.error || '❌ Failed to update announcement.'
  }
}
...
<template>
  <div class="auth-page" ref="pageRef">
    ...
    <div class="auth-card" ref="cardRef" @mousemove="onMouseMove" @mouseleave="onMouseLeave">
      ...
      <!-- heading -->
      <h2 class="card-title">📢 {{ t('editAnnouncement') || 'Edit Announcement' }}</h2>
      <p class="card-sub">
        {{ t('editAnnouncementSubtitle') || 'Update the announcement details below.' }}
      </p>

      <!-- alerts -->
      <Transition name="shake-fade">
        <div v-if="saveSuccess" class="success-alert">✅ Announcement updated! Redirecting...</div>
      </Transition>
      <Transition name="shake-fade">
      ...
      <Transition name="shake-fade">
        <div v-if="announcementStore.loading" class="info-alert">
          <span class="mini-spin"></span>
          <span>{{ t('updatingAnnouncement') || 'Updating announcement...' }}</span>
        </div>
      </Transition>

      <!-- loading state -->
      <div v-if="!announcementForm" class="loading-msg">
        <span class="mini-spin"></span>
...
.btn-submit {
  ...
  transition:
    ...
    box-shadow 0.3s;
  margin-top: 4px;
  animation: fadeUp 0.6s ease both 0.62s;
}
.btn-submit::before {
  content: '';
  position: absolute;
